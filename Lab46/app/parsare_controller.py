...
def _valideaza_si_converteste_pachet(params):
    '''
    valideaza datele introduse de catre utilizator
    :param params: datele introduse de utilizator in parsare
    :return: return la pachetul convertit
    '''
    if len(params) != 4:
...
def _valideaza_date_interval(params):
    '''
    valideaza datele introduse de catre utilizator pentru intervalul de timp al pachetului
    :param params: datele introduse de utilizator in parsare, sunt 2 string uri
    :return: se da return la cele 2 date convertite in stilul data_time
    '''
    if len(params) != 2:
...
def run_parsing():
    ...
    while True:
        ...
        for comanda_bruta in comenzi_de_pe_linie:
            ...
            comanda = parti[0].lower().strip()
            params = parti[1:]

            try:
                # Comenzi de control
                if comanda == 'exit' or comanda == '0':
                ...
                # Ii o operatiune care modifica lista de undo
                operatii_cu_modificari = ['add', 'sterge_durata', 'sterge_destinatie', 'sterge_pret']
                if comanda in operatii_cu_modificari:
                ...
                # Stergeri
                elif comanda == 'sterge_destinatie':
                ...
                elif comanda == 'sterge_durata':
                    if len(params) != 1:
                        raise ValueError("Comanda 'sterge_durata' necesita 1 parametru (numar zile).")
                    durata = int(params[0])
                ...
                # Filtrari
                elif comanda == 'filtrare_pret_dest':
                    if len(params) != 2: raise ValueError(
